# Cleanup Script - Delete CrowdStrike Connector Infrastructure
...
if (-not (Test-Path "generated-config.json")) {
    ...
    Write-Host "Run .\deploy.ps1 first to create resources." -ForegroundColor Yellow
    exit 1
}
...
$dceResourceId = $generated.dce_resource_id
$dceName = $dceResourceId -replace ".*/", ""
$dcrName = $dcrResourceId -replace ".*/", ""

# Determine cloud environment from location
$cloudEnv = if ($location -like "*gov*") { "AzureUSGovernment" } else { "AzureCloud" }
$baseUrl = if ($cloudEnv -eq "AzureUSGovernment") { "https://management.usgovcloudapi.net" } else { "https://management.azure.com" }

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "CrowdStrike Connector Cleanup" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  • Data Collection Endpoint (DCE): $dceName" -ForegroundColor White
Write-Host ""
...
Write-Host "`n[1/4] Deleting 5 data connectors..." -ForegroundColor Cyan
$connectors = @(
    "CrowdStrikeVulnerabilitiesPollerGCCH",
    "CrowdStrikeAlertsPollerGCCH",
    "CrowdStrikeIncidentsPollerGCCH",
    "CrowdStrikeDetectionsPollerGCCH",
    "CrowdStrikeHostsPollerGCCH"
)
...
Write-Host "`n[2/4] Deleting connector definition..." -ForegroundColor Cyan
...
Write-Host "`n[3/4] Deleting Data Collection Rule..." -ForegroundColor Cyan
$dcrUrl = "$baseUrl$dcrResourceId`?api-version=2022-06-01"
...
# Delete DCE using REST API with full resource ID
Write-Host "`n[4/4] Deleting Data Collection Endpoint..." -ForegroundColor Cyan
$dceUrl = "$baseUrl$dceResourceId`?api-version=2022-06-01"
$result = az rest --method DELETE --url $dceUrl 2>&1
if ($LASTEXITCODE -eq 0 -or $result -like "*NotFound*") {
    Write-Host "  ✓ Deleted DCE: $dceName" -ForegroundColor Green
} else {
    Write-Host "  ⚠ DCE deletion failed" -ForegroundColor Yellow
}
...
# Check connectors
$connectorsUrl = "$baseUrl$workspaceId/providers/Microsoft.SecurityInsights/dataConnectors?api-version=2023-02-01"
$remainingConnectors = az rest --method GET --url $connectorsUrl 2>&1 | ConvertFrom-Json
$crowdstrikeConnectors = $remainingConnectors.value | Where-Object { $_.name -like "*CrowdStrike*" }
if ($crowdstrikeConnectors) {
    ...
    $crowdstrikeConnectors | ForEach-Object { Write-Host "    - $($_.name)" -ForegroundColor Yellow }
} else {
    Write-Host "  ✓ All 5 connectors deleted" -ForegroundColor Green
}

# Check DCR
$dcrCheckUrl = "$baseUrl$dcrResourceId`?api-version=2022-06-01"
...
# Check DCE
$dceCheckUrl = "$baseUrl$dceResourceId`?api-version=2022-06-01"
$dceCheck = az rest --method GET --url $dceCheckUrl 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Host "  ⚠ Warning: DCE still exists" -ForegroundColor Yellow
} else {
    Write-Host "  ✓ DCE deleted" -ForegroundColor Green
}
